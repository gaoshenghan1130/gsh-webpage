...
export default function LeftArcDecoration() {
  ...
  useEffect(() => {
    setOffsetsX(Array.from({ length: layerCount }, () => Math.random() * 20 - 10))
    setOffsetsY(Array.from({ length: layerCount }, () => Math.random() * 200 - 100))
    setOffsetsR(Array.from({ length: layerCount }, () => Math.random() * 10))
    setOffsetH(Array.from({ length: layerCount }, () => Math.random() * 50 - 25))
  }, [])
  ...
  useEffect(() => {
    const handleMouseMove = () => {
      ...
      const newX = offsetsX.map(
        (x, i) => -radius + visibleWidth + x + waveFactorX.get() * ((i - 2) / 4) * 0.08
      )
      setWaveX(newX)
      const newY = offsetsY.map((y, i) => 50 + y * 0.2 + waveFactorY.get() * ((i - 2.5) / 4) * 0.3)
      setWaveY(newY)
      const newWidths = offsetsR.map((r, i) => radius + r + waveFactorR.get() * ((i - 2) / 4) * 0.3)
      setWaveWidths(newWidths)
      const newH = offsetsH.map((h, i) => h + radius + waveFactorH.get() * ((i - 2) / 4) * 0.03)
      setWaveHeights(newH)
    }
